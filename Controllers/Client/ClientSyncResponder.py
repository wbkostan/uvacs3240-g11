...
class SyncResponder():
    ...
    def _listen(self):
        ...
        while(self.listen_flag.is_set()):
            try:
                ...
                self.logger.log("INFO","Sync Directive received: " + str(msg[:-1]))
    ...
    def pause(self):
        self.logger.log("INFO","Responder has stopped listening over all ports")
        self.listen_flag.clear()
    def teardown(self):
        self.logger.log("INFO","Killing responder...")
        self.listen_flag.clear()
    ...
    def on_mkdir(self, msg):
        dest_path = self.config["PATH_BASE"] + msg[1]
        self.logger.log("INFO","Make directory command received, processing...")
        if(os.path.isdir(dest_path)):
            self.logger.log("INFO","Directory already exists, ignoring make command...")
            return
        else:
    ...
    def on_remove(self, msg):
        dest_path = self.config["PATH_BASE"] + msg[1]
        self.logger.log("INFO","Remove command received, processing...")
        if not os.path.exists(dest_path):
            self.logger.log("ERROR", dest_path + " does not exist. Can't remove.")
            return
        if(os.path.isdir(dest_path)):
    ...
    def on_move(self, msg):
        self.logger.log("INFO","Move command received, processing...")
        src_path = self.config["PATH_BASE"] + msg[1]
        dest_path = self.config["PATH_BASE"] + msg[2]
        if not os.path.exists(src_path):
            self.logger.log("ERROR","File system object at " + dest_path + " does not exist. Cannot move")
            return
